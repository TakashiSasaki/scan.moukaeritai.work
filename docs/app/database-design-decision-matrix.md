...
| Bluetooth tag document ID strategy | Decided | Raw ID vs. UUIDv5 | Use deterministic UUIDv5 identifier key derived from JCS payload. The payload must not include ownerId or legacyItemId. Collision checks are global. | No | Phase 7D.3, Deterministic UUID Policy |
