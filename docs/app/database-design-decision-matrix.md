...
| IdentifierRecord.objectId compatibility | Compatibility caveat / needs-decision | keep direct `identifiers.objectId` assumptions vs. migrate relation assumptions fully to bindings | `objectId` is not part of identifier identity; canonical relations should live in bindings. Audit and eventually migrate direct object relation assumptions to bindings. | Yes if direct `objectId` dependence remains. | Phase 7D.4, Ownerless Global Identifier Model |
