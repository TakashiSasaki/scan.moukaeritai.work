...
| Identifier runtime implementation path | Needs-decision | optional `ownerId` only vs. optional `ownerId` plus `identityScope` vs. optional `ownerId` plus `identityModelVersion` vs. global identifiers plus claims vs. separate global identifier registry | Use the Phase 7D.4 implementation impact audit to select the runtime implementation path. | Yes | Phase 7D.4, Ownerless Global Identifier Model |
...
| Global identifier access policy | Needs-decision | public signed-in read vs. community scoped vs. redacted values vs. claim-mediated visibility | Design read/write visibility policy before implementation. | Yes, if global identifiers are written or exposed | New |
...
| idPurpose removal from identifier payload | Decided | Remove `idPurpose` from canonical identifier payload | `idKind = "identifier"` is sufficient purpose separation | No for concept; Yes only if stale docs remain | Phase 7D.5 |
| identityModelVersion | Decided conceptually; implementation pending | Introduce runtime interpretation version on `IdentifierRecord`; exclude from UUIDv5 payload | Missing or `1` = legacy/current model, `2` = ownerless/global model | Yes (implementation validation) | Phase 7D.5 |
| rawPayload (future v2) | Decided conceptually; implementation pending | Use optional `rawPayload?: JsonValue`; do not use `rawValue` in future v2 design | Keep identity derivation independent from source payload snapshots | Yes (mapping validation) | Phase 7D.5 |
| scheme in semantic identity | Decided | Keep `scheme` as part of identifier identity | `identityModelVersion` does not define canonical value namespace | No | Phase 7D.5 |
| ACL future-proofing | Deferred by design | Do not add ACL-specific fields now; preserve evidence for future ACL derivation | Avoid premature ACL model lock-in | No for docs task | Phase 7D.5 |
| identifierClaims | Future-only | Do not implement now and do not require for Phase 7D.5/7D.6 | Not prerequisite for current documentation/design phases | No | Phase 7D.5 |
