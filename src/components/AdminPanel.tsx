...
import { runImportedObservationDryRun, ImportedObservationDryRunResult } from '../lib/importedObservationDryRun';
...
export default function AdminPanel({ onClose }: { onClose?: () => void }) {
  ...
  const [importedDryRunRunning, setImportedDryRunRunning] = useState(false);
  const [importedDryRunResult, setImportedDryRunResult] = useState<ImportedObservationDryRunResult | null>(null);
  const [importedDryRunError, setImportedDryRunError] = useState<string | null>(null);
  ...
  const handleRunImportedDryRun = async () => {
    if (!auth.currentUser) return;
    setImportedDryRunRunning(true);
    setImportedDryRunError(null);
    setImportedDryRunResult(null);

    try {
      const result = await runImportedObservationDryRun(db, auth.currentUser.uid, {
        maxIdentifiers: 50,
        maxObservationsPerIdentifier: 20,
        maxCandidates: 50,
        maxSamplesPerCategory: 5
      });
      setImportedDryRunResult(result);
    } catch (err: any) {
      setImportedDryRunError(err.message || 'Unknown error during imported observation dry run');
    } finally {
      setImportedDryRunRunning(false);
    }
  };
    ...
    <div className="w-full">
      ...
      <div className="p-4 sm:p-6 pb-24 max-w-5xl mx-auto space-y-6">
        <div className="space-y-6">
            ...
            {/* Imported Observation Dry Run Section */}
            <div className="bg-[var(--surface-container-high)] rounded-3xl p-6 lg:p-8 border border-[var(--outline)]">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-bold text-[var(--on-surface)] flex items-center gap-2">
                  <Database size={20} className="text-blue-500" />
                  インポート観測 Dry Run (Imported Observation)
                </h3>
                <button
                  onClick={handleRunImportedDryRun}
                  disabled={importedDryRunRunning}
                  className="px-4 py-2 bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white rounded-xl text-sm font-bold transition-colors flex items-center gap-2 shadow-sm"
                >
                  {importedDryRunRunning ? <Loader2 size={16} className="animate-spin" /> : <Activity size={16} />}
                  Dry Run を実行 (Run)
                </button>
              </div>
              <p className="text-sm text-[var(--on-surface-variant)] mb-6">
                Read-only, bounded dry-run planner for potential imported baseline observations based on the current authenticated user’s scoped records. This calculates proposed updates without writing any data.
                <br />
                <span className="block mt-2 text-[13px] text-[var(--on-surface-variant)]">
                  「現在のログインユーザーに紐づく範囲を対象とした、読み取り専用のインポート観測 Dry Run です。既存の識別子データからベースラインとなる観測記録の候補を生成しますが、実際のデータ書き込み（変更）は一切行いません。」
                </span>
                <br />
                <span className="font-bold text-blue-600">Note:</span> This phase computes proposals with deterministic UUIDv5 IDs. It does NOT create imported observations, execute updates, or implement repairs.
              </p>
              ...
              {importedDryRunResult && (
                <div className="space-y-6">
                  {/* Counts */}
                  <div className="grid grid-cols-2 gap-4">
                    <div className="p-3 bg-[var(--surface)] border border-[var(--outline)] rounded-xl text-center">
                      <div className="text-xs text-[var(--on-surface-variant)] font-bold mb-1">Identifiers Checked</div>
                      <div className="text-xl font-black text-[var(--on-surface)]">{importedDryRunResult.counts.identifiersChecked}</div>
                    </div>
                    <div className="p-3 bg-[var(--surface)] border border-[var(--outline)] rounded-xl text-center">
                      <div className="text-xs text-[var(--on-surface-variant)] font-bold mb-1">Candidates Proposed</div>
                      <div className="text-xl font-black text-[var(--on-surface)]">{importedDryRunResult.candidateCounts}</div>
                    </div>
                  </div>
                  ...
                  <div>
                    <h4 className="text-md font-bold text-[var(--on-surface)] mb-3 flex items-center gap-2">
                      <Activity size={18} className={importedDryRunResult.candidates.length > 0 ? "text-blue-500" : "text-emerald-500"} />
                      Candidates (Proposals): {importedDryRunResult.candidates.length}
                    </h4>
                    {importedDryRunResult.candidates.length === 0 ? (
                      <div className="p-4 bg-emerald-50 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-400 rounded-xl border border-emerald-200 dark:border-emerald-800 text-sm font-medium">
                        No imported observation candidates found.
                      </div>
                    ) : (
                      <div className="space-y-4">
                        {importedDryRunResult.candidates.map((candidate, idx) => (
                          <div key={idx} className="p-4 bg-[var(--surface)] border border-[var(--outline)] rounded-xl">
                            <div className="flex items-start gap-3">
                              <div className="flex-1 min-w-0">
                                <div className="font-bold text-sm text-[var(--on-surface)]">
                                  Identifier: {candidate.identifierKey}
                                </div>
                                <div className="text-xs text-[var(--on-surface-variant)] mt-1">Reason: {candidate.reason}</div>
                                <div className="text-xs text-[var(--on-surface-variant)] mt-1 font-mono break-all">ID: {candidate.observationId}</div>
                                <div className="mt-3 space-y-2">
                                  <div className="text-xs font-bold text-[var(--on-surface-variant)]">Proposed Payload Preview:</div>
                                  <pre className="text-[10px] bg-[var(--surface-container-highest)] p-2 rounded-lg overflow-x-auto text-[var(--on-surface)]">
                                    {JSON.stringify(candidate.proposedObservation, null, 2)}
                                  </pre>
                                </div>
                              </div>
                            </div>
                  ...
                  {/* Skipped */}
                  {importedDryRunResult.skipped.length > 0 && (
                    <div>
                      <h4 className="text-md font-bold text-[var(--on-surface)] mb-3 flex items-center gap-2">
                        <CloudCog size={18} className="text-amber-500" />
                  ...
                  {/* Warnings */}
                  {importedDryRunResult.warnings.length > 0 && (
                    <div>
                      <h4 className="text-md font-bold text-[var(--on-surface)] mb-3 flex items-center gap-2">
                        <AlertCircle size={18} className="text-red-500" />
                        Warnings: {importedDryRunResult.warnings.length}
                      </h4>
                      <div className="space-y-4">
