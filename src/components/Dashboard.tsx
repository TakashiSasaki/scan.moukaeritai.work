        ...
        {items.length === 0 ? (
          ...
          <div className="grid grid-cols-1 gap-4">
            {items.map((item) => (
              ...
              >
                {item.mainImageUrl ? (
                  <img src={item.mainImageUrl} alt={item.name} className="w-24 h-24 rounded-2xl object-cover flex-shrink-0 border border-[var(--outline)] shadow-sm" referrerPolicy="no-referrer" />
                ) : (
