...
for (const version of compatibleWith) {
  for (const name of ['association','observation','measurement','event']) {
    const file = path.join(rootDir, 'contracts/fixtures/compatibility', version, 'valid', `${name}.json`);
    if (!fs.existsSync(file)) throw new Error(`Missing version-specific compatibility fixture: ${version}/valid/${name}.json`);
    assertAccept(file, `${version}/${name}.json`);
  }
}
const base = JSON.parse(fs.readFileSync(path.join(rootDir, 'contracts/fixtures/compatibility', compatibleWith[0], 'valid', 'association.json'), 'utf8'));
