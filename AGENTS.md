...
- **PWA Icon Assets**: The source app icon is `public/icon.svg`; installable icon assets are SVG-only and live in `public/favicon.svg`, `public/apple-touch-icon.svg`, `public/pwa-icon-192.svg`, `public/pwa-icon-512.svg`, and `public/maskable-icon.svg`. Update `vite.config.ts` manifest declarations and `index.html` links together when changing these files.
